    ...
    elseif(LP(iLayer)%Level.gt.1) then !do not calculate GHOST grids in child layers
    ...
    do i = istart,iend
        ...
        !/// flux-centered ///!
        if(LP(iLayer)%FluxCenter.eq.1) phi = 1.0 - LP(iLayer)%CC1*MIN(ABS(m0/MAX(D0,GP%MinWaterDepth)),SQRT(GP%GRAV*D0))
        ...
        if(FluxSign.ne.0.and.LP(iLayer)%M(2,i,j)*FluxSign.lt.0.0) LP(iLayer)%M(2,i,j) = 0.0d0
    enddo
    enddo
    ...
    do i = istart,iend
        ...
        !/// flux-centered ///!
        if(LP(iLayer)%FluxCenter.eq.1) phi = 1.0 - LP(iLayer)%CC2*MIN(ABS(n0/MAX(D0,GP%MinWaterDepth)),SQRT(GP%GRAV*D0))
        ...
        if(FluxSign.ne.0.and.LP(iLayer)%N(2,i,j)*FluxSign.lt.0.0) LP(iLayer)%N(2,i,j) = 0.0d0
    enddo
    enddo
    ...
    do i = istart,iend
        ...
        !/// flux-centered ///!
        if(LP(iLayer)%FluxCenter.eq.1) phi = 1.0 - LP(iLayer)%CS1(j)*MIN(ABS(m0/MAX(D0,GP%MinWaterDepth)),SQRT(GP%GRAV*D0))
    ...
    do i = istart,iend
        ...
        !///flux-centered///!
        if(LP(iLayer)%FluxCenter.eq.1) phi = 1.0 - LP(iLayer)%CS5*MIN(ABS(n0/MAX(D0,GP%MinWaterDepth)),SQRT(GP%GRAV*D0))
